...
Despite their common conceptual structure, exchange data feeds differ in message formats, order type definitions, and exchange rules which affect processing logic. A single day of Nasdaq TotalView ITCH data can exceed ten gigabytes in compressed binary form and contain billions of messages. Processing these heterogeneous and high-volume data streams has traditionally relied on custom scripts kept private by research teams or on proprietary third-party platforms. This situation limits reproducibility and slows research progress.
...
Several tools exist for processing limit order book data. LOBSTER [@huangLOBSTERLimitOrder2011] is a widely-used commercial platform that reconstructs order books from Nasdaq ITCH data as an online service, but its closed-source nature limits extensibility and reproducibility. Other published implementations [@clarkmcgeheeObjectOrientedLibraryRealTime2013; @gaiFastConstructionNanosecond2013] were not released as reusable open-source libraries.

Prior to MeatPy, co-author Martineau maintained a proprietary SAS-based solution for order book reconstruction. This implementation proved difficult to maintain and extend due to SAS's limited support for object-oriented programming. Additionally, SAS's proprietary licensing created barriers to deploying analyses on high-performance computing facilities, where license management adds complexity and cost.

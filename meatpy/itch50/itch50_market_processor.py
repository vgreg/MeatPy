...
class ITCH50MarketProcessor(MarketProcessor):
        ...
        if (isinstance(message, AddOrderMessage) or
            ...
            if self.track_lob:
                if message.bsindicator == b'B':
                    ...
                    raise Exception('ITCH50MarketProcessor:process_message',
                                    'Wrong value for bsindicator: ' + str(message.bsindicator))
                self.pre_lob_event(timestamp)
                self.enter_quote(timestamp=timestamp,
                                 price=message.price,
            ...
            if self.track_lob:
                self.pre_lob_event(timestamp)
                self.execute_trade(timestamp=timestamp, volume=message.shares,
                                   order_id=message.orderRefNum,
            ...
            if self.track_lob:
                self.pre_lob_event(timestamp)
                self.cancel_quote(timestamp=timestamp,
                                  volume=message.cancelShares,
                                  order_id=message.orderRefNum)
        elif isinstance(message, OrderDeleteMessage):
            if self.track_lob:
                self.pre_lob_event(timestamp)
                self.delete_quote(timestamp=timestamp,
                                  order_id=message.orderRefNum)
            ...
            if self.track_lob:
                ...
                if self.current_lob.ask_order_on_book(message.origOrderRefNum):
                    order_type = 0
                elif self.current_lob.bid_order_on_book(
                        message.origOrderRefNum):
                    order_type = 1
                else:
                    raise Exception('ITCH50MarketProcessor:process_message',
                                    'Replacing missing order.')
                self.replace_quote(timestamp=timestamp,
            ...
            self.process_system_message(message.code, timestamp,
                                        new_snapshot)
        elif isinstance(message, StockTradingActionMessage):
            self.process_trading_action_message(message.state, timestamp,
                                                new_snapshot)
    ...
    def process_system_message(self, code, timestamp, new_snapshot=True):
        ...
        if code in b'OSQMEC':
            ...
            raise Exception('ITCH50MarketProcessor:process_system_message',
                            'Unknown system message: ' + str(code))
        self.update_trading_status()

    def process_trading_action_message(self, state, timestamp,
                                       new_snapshot=True):
        """Process a stock trading action message"""
        if state in b'HPQT':
            self.stock_status = state
        else:
            raise Exception('ITCH50MarketProcessor:process_trading_action_message',
    ...
    def update_trading_status(self):
        ...
        if self.emc_status == b'A' or self.stock_status in b'HP':
            ...
            raise Exception('ITCH50MarketProcessor:update_trading_status',
                            'Could not determine trading status: ' +
                            str(self.system_status) + '/' + str(self.emc_status) + '/' +
    ...
    def enter_quote(self, timestamp, price, volume, order_id, order_type):
        ...
        self.current_lob.enter_quote(timestamp, price, volume, order_id,
                                     order_type)
    ...
    def replace_quote(self, timestamp, orig_order_id, new_order_id, price,
                      volume, order_type):
        """Replace a quote in the LOB."""
        #
        # Replace an order, so basilly remov previous instance and introduce
        # new one.
        self.replace_quote_event(timestamp, orig_order_id, new_order_id,
                                 price, volume, order_type)
        self.current_lob.delete_quote(orig_order_id)
        self.current_lob.enter_quote(timestamp, price, volume, new_order_id,
                                     order_type)

    def execute_trade(self, timestamp, volume, order_id,
                      trade_ref):
    ...
    def execute_trade_price(self, timestamp, volume, order_id,
        ...
        # Execute order from new snapshot, so the snapshot can apply
        # the appropriate ranking according to rules
        self.execute_trade_price_event(timestamp, volume, order_id, trade_ref,
                                       price)
